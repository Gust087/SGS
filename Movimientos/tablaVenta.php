...
function selectProducto( precio_vta, existencia, nombre, codigo, marca){			
//	alert(nombre);
//	alert(marca.ToString());
	rellenarInputs(nombre,codigo, marca);
	rellenarPrecio(precio_vta);
	rellenarCantidad(existencia);
	rellenarTotal();
}
...
function rellenarPrecio(precio_vta){
	document.getElementById("p_c").value = precio_vta;
}

function rellenarCantidad(existencia){
	if(existencia == 0){
		alert("No se puede vender este producto. Stock: "+ existencia );
	}else{
		alert("Maximo a vender de este producto es: "+ existencia );
		}

		document.getElementById("cant").value = 1;
		var elemento = document.querySelector('#cant');
		elemento.setAttribute('max', existencia);
}
...
function restar(){
	total = parseInt (contador) - parseInt (500);
	window.opener.document.formulario.balance.value = total;
	contador = total;
}
...
function agregar_lista_producto(){
	...
	ProductosVentados = window.opener.document.getElementById("cargarItems");
	...
	ProductosVentados.appendChild(linea);
	id ++;
	}
}
...
<?php 
  ...
  $r_query = mysqli_query($conexion, $query) or die("Failed to execute_query");
        ...
        while ($row = mysqli_fetch_array($r_query)) {
            ...
            //Anotacion: this ? =...".$precio_vta.", ".$existencia.", ".$min." ...'$codigo','$nombre','$marca','$modelo','$precio_vta','$precio_vta','$existencia','$descripcion','$max','$min','$id'
?>			
			<tr OnClick="selectProducto(<?=$precio_vta?>, <?=$existencia?>, '<?=$nombre?>', <?=$codigo?>,'<?=$marca?>');" OnMouseOver="Resaltar_On(this);" OnMouseOut="Resaltar_Off(this);" >
			<td><?=$codigo?></td>
			<td><?=$nombre?></td>
			<td><?=$marca?></td>
			<td><?=$modelo?></td>
...
<div>
		...
		<table>
                <tr>
                    <td><b>Cantidad:</b></td>
					<td><input width="80px" id='cant' name='cant' type='number' min="1" required ></td>
				</tr>
				<tr>
					<td><b>Precio Contado:</b></td>
					<td><input width="80px" id='p_c' name='p_c' type='number' required ></td>
				</tr>
		</table>
		</div>
		<div>
