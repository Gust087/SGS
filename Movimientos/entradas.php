...
$_pagi_sql = "SELECT * FROM entradas WHERE fecha BETWEEN '$f_in' AND '$f_fi'";
$result = mysqli_query($conexion, $_pagi_sql);
$total_registros = mysqli_num_rows($result);
$result = mysqli_query($conexion, "SELECT * FROM entradas WHERE fecha BETWEEN  '$f_in' AND '$f_fi' ORDER BY fecha DESC LIMIT $inicio, $registros");
...
<body>
	...
	<form method="POST" name="formulario" action="index.php">
		<fieldset  id="cuadro" >
			<legend id="nombre_tabla">Entradas</legend>
			<table id="tabla">
				<tr>
...
?>
	...
	<!--?php// print $_pagi_navegacion;?-->
		<?php
	if(($pag - 1) > 0) {
		echo "<a href='index.php?pagina=entradas&pag=".($pag-1)."&f_inicial=".$f_in."&f_final=".$f_fi."'>< Anterior </a>";} 
	for ($i=1; $i<=$total_paginas; $i++){
		if ($pag == $i) {
			echo "<b>".$pag."</b> ";} 
		else {
		if(($pag-1) == $i OR ($pag+1) == $i){
			echo "<a href='index.php?pagina=entradas&pag=".$i."&f_inicial=".$f_in."&f_final=".$f_fi."'>".$i."</a> ";} 
		}
	}
	if(($pag + 1)<=$total_paginas) {
		echo "<a href='index.php?pagina=entradas&pag=".($pag+1)."&f_inicial=".$f_in."&f_final=".$f_fi."'>Siguiente ></a>";} 
		$f_in= str_replace("-", "", $f_in);//se les borra los guiones medios.
		$f_fi= str_replace("-", "", $f_fi);//se les borra los guiones medios.
?>
		<button style="margin-left:200px;" type="submit" name="sali" id="button" onclick="imprimirMov('entr',<?php echo"".$f_in."";?>,<?php echo"".$f_fi."";?>);">Imprimir</button></div>
		</form>
</div>
</section>
</body>
