 ...
 */
 ...
 if($_pagi_conteo_alternativo == false){
 	//$_pagi_sqlConta = preg_replace('/ .* /', ' COUNT .* FROM ', $_pagi_sql);
	//$_pagi_sqlConta = str_replace ( "." ,  '' , $_pagi_sqlConta );
	$_pagi_result2 = mysqli_query($conexion, $_pagi_sql);
	// Si ocurrió error y mostrar errores está activado
 	if($_pagi_result2 == false && $_pagi_mostrar_errores == true){
		die (" Error en la consulta de conteo de registros: $_pagi_sqlConta. Mysql dijo: <b>".mysqli_error($conexion)."</b>");
 	}
 ...
 */
 ...
 if($_pagi_result == false && $_pagi_mostrar_errores == true){
 	die ("Error en la consulta limitada: $_pagi_sqlLim. Mysql dijo: <b>".mysqli_error($conexion)."</b>");
 }
